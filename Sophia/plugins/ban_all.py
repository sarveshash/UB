...
async def unbanall(_, message):
    ...
    if user_id != OWN and (await is_admin(chat_id, user_id)) == False:
        return await message.reply("`Sorry, I don't have admin rights to do this.`")
        ...
        try:
            ...
            async for m in bot.get_chat_members(chat_id, filter=enums.ChatMembersFilter.BANNED):
                ...
                try:
                    ...
                    failed_unban += 1
            reply_text = (
                f"**Found Banned Members**: `{len(BANNED)}`\n"
                f"**Unbanned Successfully**: `{unban}`\n"
                f"**Failed to Unban**: `{failed_unban}`"
            )
